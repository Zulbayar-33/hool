...
function createGraphicSheet(){
  ...
  const configs=[{sheet:FIT_SHEET,key:"FitFood",price:15000},{sheet:SERVICE_SHEET,key:"ServiceFood",service:true},{sheet:HOOL_SHEET,key:"Hool",price:15000},{sheet:BREAKFAST_SHEET,key:"OglooniiTsai",price:0}];
  ...
  const totalCount=summary.FitFood.count+summary.ServiceFood.count+summary.Hool.count+summary.OglooniiTsai.count,totalFood=summary.FitFood.count+summary.Hool.count,totalMoney=Object.keys(summary).reduce((s,k)=>s+summary[k].money,0);
  ...
  const summaryRows=[["FitFood",summary.FitFood.count,15000,summary.FitFood.money],["ServiceFood",summary.ServiceFood.count,"Үнийн дүнгээр",summary.ServiceFood.money],["Хоол",summary.Hool.count,15000,summary.Hool.money],["Өглөөний цай",summary.OglooniiTsai.count,"ServiceFood-д багтсан",0],["Нийт",totalCount,"Нийт хоол: "+totalFood,totalMoney]];
