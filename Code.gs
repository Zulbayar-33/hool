...
function getSpreadsheet_(){const active=SpreadsheetApp.getActiveSpreadsheet();if(!active)throw new Error("Apps Script-ийг үндсэн хоолны Google Sheet-ээс нээнэ үү");return active;}
...
function saveFood(data){
  ...
  try{
    ...
    const existing=String(sh.getRange(row,col).getDisplayValue()||"").trim();
    let value;
    if(type==="service")value=(Number(existing)||0)+amount;
    else if((type==="fit"||type==="hool"||type==="ahool")&&isAfterCutoff_())value="Pending:"+((Number(existing)||pendingCount_(existing))+1);
    else value=(Number(existing)||0)+1;
    sh.getRange(row,1,1,4).setValues([[no,String(data.ovog||""),ner,String(data.job||"")]]);sh.getRange(row,col).setValue(value);
    updateRowTotal(sh,row);updateTotalRow(sh);if(sheetName===BREAKFAST_SHEET)recalculateBreakfastPrices_();createTotalSheet();createGraphicSheet();const pending=String(value).indexOf("Pending")===0;writeLog_(pending?"PENDING_CREATED":"FOOD_SAVED",data,(pending?"Admin зөвшөөрөл хүлээж байна • Тоо: "+pendingCount_(value):"Нийт: "+value+" • "+date),sheetName,row,col);
    return{success:true,pending:String(value).indexOf("Pending:")===0,message:String(value).indexOf("Pending:")===0?"11:30 өнгөрсөн тул Admin зөвшөөрөл хүлээж байна":sheetName+" амжилттай хадгалагдлаа"};
...
function getAdminDashboard(){
  ...
  [FIT_SHEET,SERVICE_SHEET,HOOL_SHEET,BREAKFAST_SHEET].forEach(name=>{const sh=ss.getSheetByName(name);if(!sh)return;const data=sh.getDataRange().getDisplayValues();if(data.length<2)return;
    for(let r=1;r<data.length;r++){if(String(data[r][0]).trim()==="Нийт")continue;for(let c=4;c<data[0].length;c++){const v=String(data[r][c]||"").trim();if(!v||data[0][c]==="Нийт"||!isDateInCurrentPeriod_(data[0][c]))continue;const pending=v.indexOf("Pending")===0;rows.push({sheet:name,row:r+1,col:c+1,no:data[r][0],ovog:data[r][1],ner:data[r][2],job:data[r][3],date:data[0][c],value:v,pending:pending});stats.all++;if(pending)stats.pending++;const n=Number(v)||0;if(name===FIT_SHEET)stats.fit+=n;if(name===SERVICE_SHEET)stats.service+=n;if(name===HOOL_SHEET)stats.hool+=n;if(name===BREAKFAST_SHEET)stats.breakfast+=n;}}}
  );stats.breakfastMoney=stats.breakfast*12000;stats.totalFood=stats.fit+stats.hool+stats.breakfast;stats.totalMoney=((stats.fit+stats.hool)*15000)+stats.service+stats.breakfastMoney;rows.sort((a,b)=>b.col-a.col||b.row-a.row);return{success:true,rows,stats,employees:getAdminEmployees_(),logs:getRecentLogs_(80)};
}

function getAdminEmployees_(){const sh=getSpreadsheet_().getSheetByName(EMP_SHEET);if(!sh||sh.getLastRow()<2)return[];return sh.getRange(2,1,sh.getLastRow()-1,6).getDisplayValues().filter(r=>String(r[0]||r[1]||"").trim()&&String(r[4]||"").trim()).map(r=>({no:String(r[0]||r[1]).trim(),ovog:r[3]||"",ner:r[4]||"",job:r[5]||"",label:String(r[0]||r[1]).trim()+" • "+(r[3]||"")+" "+(r[4]||"")+" • "+(r[5]||"")})).sort((a,b)=>String(a.no).localeCompare(String(b.no),"mn",{numeric:true}));}

function adminAddFood(p){const lock=LockService.getScriptLock();lock.waitLock(20000);try{const type=String(p.type||""),map={fit:FIT_SHEET,service:SERVICE_SHEET,hool:HOOL_SHEET,breakfast:BREAKFAST_SHEET},sheetName=map[type];if(!sheetName)return{success:false,message:"Хоолны төрөл буруу байна"};const employees=getAdminEmployees_(),emp=employees.find(x=>String(x.no)===String(p.employeeNo));if(!emp)return{success:false,message:"Ажилтан олдсонгүй"};const date=String(p.date||"").trim(),quantity=Math.max(1,Number(p.quantity)||1),amount=Number(p.amount)||0;if(!date)return{success:false,message:"Огноо сонгоно уу"};if(type==="service"&&amount<=0)return{success:false,message:"ServiceFood үнийн дүн оруулна уу"};const ss=getSpreadsheet_();let sh=ss.getSheetByName(sheetName);if(!sh){setupOneSheet_(ss,sheetName,true);sh=ss.getSheetByName(sheetName);}const col=ensureDateColumn(sh,date);let totalRow=findTotalRow(sh),row=findEmployeeRow(sh,emp.no,totalRow);if(!row){sh.insertRowBefore(totalRow);row=totalRow;}const raw=String(sh.getRange(row,col).getDisplayValue()||"").trim(),existing=Number(raw)||pendingCount_(raw),newValue=existing+(type==="service"?amount:quantity);sh.getRange(row,1,1,4).setValues([[emp.no,emp.ovog,emp.ner,emp.job]]);sh.getRange(row,col).setValue(newValue);updateRowTotal(sh,row);updateTotalRow(sh);if(sheetName===BREAKFAST_SHEET)recalculateBreakfastPrices_();createTotalSheet();createGraphicSheet();writeLog_("ADMIN_MANUAL_ADD",{no:emp.no,ovog:emp.ovog,ner:emp.ner},sheetName+" • "+date+" • Нэмсэн: "+(type==="service"?amount+"₮":quantity)+" • Нийт: "+newValue,sheetName,row,col);return{success:true,message:emp.ovog+" "+emp.ner+" — "+sheetName+" дээр нэмэгдлээ"};}catch(err){return{success:false,message:err.message};}finally{lock.releaseLock();}}
...
function adminAction(p){const allowed=[FIT_SHEET,HOOL_SHEET];if(!allowed.includes(String(p.sheet)))return{success:false,message:"Зөвхөн FitFood/Хоол Pending зөвшөөрнө"};const sh=getSpreadsheet_().getSheetByName(p.sheet),row=Number(p.row),col=Number(p.col),current=sh?String(sh.getRange(row,col).getDisplayValue()).trim():"";if(!sh||current.indexOf("Pending")!==0)return{success:false,message:"Pending бүртгэл олдсонгүй"};const value=p.action==="approve"?pendingCount_(current):p.action==="reject"?"Rejected":null;if(value===null)return{success:false,message:"Үйлдэл буруу"};sh.getRange(row,col).setValue(value);updateRowTotal(sh,row);updateTotalRow(sh);createTotalSheet();createGraphicSheet();writeLog_(p.action==="approve"?"ADMIN_APPROVED":"ADMIN_REJECTED",{no:sh.getRange(row,1).getDisplayValue(),ner:"Admin"},p.sheet+" • Тоо: "+pendingCount_(current),p.sheet,row,col);return{success:true,message:p.action==="approve"?"Зөвшөөрлөө":"Татгалзлаа"};}

function pendingCount_(value){const m=String(value||"").match(/^Pending(?::(\d+))?$/);return m?Math.max(1,Number(m[1])||1):0;}
