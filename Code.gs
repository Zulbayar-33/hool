...
function setupSheets(){
  ...
  setupLogSheet_(ss);recalculateBreakfastPrices_();createTotalSheet();
}
...
function saveFood(data){
  ...
  try{
    ...
    updateRowTotal(sh,row);updateTotalRow(sh);if(sheetName===BREAKFAST_SHEET)recalculateBreakfastPrices_();createTotalSheet();writeLog_(value==="Pending"?"PENDING_CREATED":"FOOD_SAVED",data,(value==="Pending"?"Admin зөвшөөрөл хүлээж байна":value+" • "+date),sheetName,row,col);
...
function getAdminDashboard(){
  const ss=SpreadsheetApp.getActiveSpreadsheet(),rows=[],stats={fit:0,service:0,hool:0,breakfast:0,breakfastMoney:0,totalFood:0,totalMoney:0,all:0,pending:0,period:getCurrentPeriod_().label};
