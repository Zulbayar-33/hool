...
4. The Apps Script project uses its original attached Google Sheet. No external spreadsheet property is required.
...
- Admin total money: `(FitFood total + Hool total) × 15,000 + ServiceFood total amount + Breakfast total × 12,000`.
- Breakfast (`OglooniiTsai`) stays as its own existing sheet: one click = 1 and its price is 12,000₮ each. Registration closes at 10:00 (Asia/Ulaanbaatar). Breakfast is included in `Total` and all graphics.
...
- Employee login and admin selection use the original `Employees` sheet.
- Repeated registrations are allowed. Count foods add to the existing quantity and ServiceFood adds to the existing money amount. No duplicate popup is shown.
